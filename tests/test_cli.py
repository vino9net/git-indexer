...
def test_cmdline_options():
    ...
    args = parse_options(
        shlex.split(
            "--mode commits --source gitlab --query stuff_that_i_care --filter '*' --mirror_path ~/tmp/mirror"  # noqa E501
        )
    )
    assert (
        args.source == "gitlab" and args.mode == "commits" and len(args.mirror_path) > 1
    )
...
def test_index_github(tmp_path, sql_engine):
    argv = shlex.split(
        f"--mode commits --source github --query sloppycoder/hello --mirror_path {tmp_path}"  # noqa E501
    )
    main(argv=argv)


def test_index_from_list(tmp_path, sql_engine):
    # the repo must be different from any other repos used in tests.
    # otherwise it'll skew the commits count
    # and cause tests to fail
    argv = shlex.split(
        f"--mode commits --source list --query tests/data/test.lst --mirror_path {tmp_path}"  # noqa E501
    )
    main(argv=argv)
...
def test_cmdline_mode_mirror(mytest_dir, sql_engine, mocker):
    ...
    argv = shlex.split(
        f"--mode mirror --source list --query {mytest_dir}/data/test.lst --mirror_path /blah"  # noqa E501
    )
    main(argv=argv)
...
def test_cmdline_mode_commits(mytest_dir, sql_engine, mocker):
    ...
    argv = shlex.split(
        f"--mode commits --source list --query {mytest_dir}/data/test.lst --mirror_path /blah"  # noqa E501
    )
    main(argv=argv)
...
def test_cmdline_mode_requests(mytest_dir, sql_engine, mocker):
    ...
    argv = shlex.split(
        f"--mode requests --source list --query {mytest_dir}/data/test.lst --mirror_path /blah"  # noqa E501
    )
    main(argv=argv)

    assert m.mirror_repo.assert_not_called()
    assert m.index_commits.assert_not_called()
    assert m.index_merge_requests.call_count == 2
