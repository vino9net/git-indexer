...
def test_index_local_repo(session, local_repo, mirror_parent_path):
    ...
    repo_obj, n_commits = index_commits(
        session, repo1, local_repo_path=repo1, repo_source="local"
    )
    assert n_commits == 2
    ...
    repo_obj, n_commits = index_commits(
        session, repo1_fork, local_repo_path=repo1_fork, repo_source="local"
    )
    ...
    # the actual commit date is "2023-07-07T15:59:06+08:00"
    # we use UTC time without timezone
    ...
    repo_obj, n_commits = index_commits(
        session, repo1_fork, repo_source="local", local_repo_path=repo1_fork
    )

    assert n_commits == n_new_commits
    assert abs((repo1_fork_obj.last_commit_at - new_commit_dt).total_seconds()) < 60  # pyright: ignore
...
def test_index_empty_repo(session, local_repo, mirror_parent_path):
    ...
    _, n_commits = index_commits(
        session, empty_repo, local_repo_path=empty_repo, repo_source="local"
    )
...
def _get_row_count_from_join_table_(session):
    result = session.execute(
        text("select count(*) from " + repo_to_commit_table.name)
    ).fetchone()
